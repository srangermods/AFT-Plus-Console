...
ReferenceAlias  Property pCompanion6      Auto Const
ReferenceAlias  Property pCompanion7      Auto Const
ReferenceAlias  Property pCompanion8      Auto Const
ReferenceAlias  Property pCompanion9      Auto Const
ReferenceAlias  Property pCompanion10      Auto Const
...
GlobalVariable Property pTweakCharismaRecruifollowerLimit				Auto Const
...
Location Property pDLC01LairLocation Auto Const
...
Event OnInit()
	...
	companions = new ReferenceAlias[10]
...
Function OnGameLoaded(bool firstTime=false)
	...
	if !version303Updated && !firstTime
		companions = new ReferenceAlias[10]
		...
		companions[5] = pCompanion6	
		companions[6] = pCompanion7	
		companions[7] = pCompanion8	
		companions[8] = pCompanion9
		companions[9] = pCompanion10
		version303Updated = true
	...
	elseif (pFollowers)
		...
		while (c < clen)
			Actor npc = companions[c].GetActorReference()
			if npc
			  if !(HasKeyword(AO_Type_Comment_128))
					AddKeywords(npc)	
				endif
				CompanionActorScript CAS = npc as CompanionActorScript
				if CAS
...
Function AftReset()
	...
	if npc
		UnManage(npc)
	endIf
	npc = pCompanion6.GetActorReference()	
	if npc
		UnManage(npc)
	endIf
	npc = pCompanion7.GetActorReference()	
	if npc
		UnManage(npc)
	endIf
	npc = pCompanion8.GetActorReference()	
	if npc
		UnManage(npc)
	endIf
	npc = pCompanion9.GetActorReference()	
	if npc
		UnManage(npc)
	endIf
	npc = pCompanion10.GetActorReference()	
	if npc
		UnManage(npc)
	endIf
	npc = pDogmeatCompanion.GetActorReference()	
	...
	UnregisterForRemoteEvent(pCompanion1.getActorReference(), "OnLocationChange")
	UnregisterForRemoteEvent(pCompanion2.getActorReference(), "OnLocationChange")
	UnregisterForRemoteEvent(pCompanion3.getActorReference(), "OnLocationChange")
	UnregisterForRemoteEvent(pCompanion4.getActorReference(), "OnLocationChange")
	UnregisterForRemoteEvent(pCompanion5.getActorReference(), "OnLocationChange")
	UnregisterForRemoteEvent(pCompanion6.getActorReference(), "OnLocationChange")
	UnregisterForRemoteEvent(pCompanion7.getActorReference(), "OnLocationChange")
	UnregisterForRemoteEvent(pCompanion8.getActorReference(), "OnLocationChange")
	UnregisterForRemoteEvent(pCompanion9.getActorReference(), "OnLocationChange")
	UnregisterForRemoteEvent(pCompanion10.getActorReference(), "OnLocationChange")
	UnregisterForRemoteEvent(pDogmeatCompanion.getActorReference(), "OnLocationChange")
...
Event Actor.OnLocationChange(Actor player, Location akOldLoc, Location akNewLoc)
	  ...
	  ; Cheap check to avoid costly script jumps...
		...
		if akNewLoc == pDLC01LairLocation
...
Function CommandModeFigureItOut(ObjectReference closestLocked, keyword requiredkeyword, bool usePickLock = true)
	...
	c = pCompanion6.GetActorReference()
	if c && c.Is3DLoaded() && c.HasKeyWord(requiredkeyword) 
		if !c.IsDead() && !c.IsBleedingOut() && !c.IsUnconscious()
			if (c.GetValue(Paralysis) != 1) && !c.IsInFaction(pTweakPosedFaction) && !c.IsDoingFavor()
				if (c.GetCurrentPackage() != pCommandMode_Travel) && (c.GetValue(pFollowerState) != 2.0)
...
c = pCompanion7.GetActorReference()
	if c && c.Is3DLoaded() && c.HasKeyWord(requiredkeyword) 
		if !c.IsDead() && !c.IsBleedingOut() && !c.IsUnconscious()
			if (c.GetValue(Paralysis) != 1) && !c.IsInFaction(pTweakPosedFaction) && !c.IsDoingFavor()
				if (c.GetCurrentPackage() != pCommandMode_Travel) && (c.GetValue(pFollowerState) != 2.0)
	...
	c = pCompanion8.GetActorReference()
	if c && c.Is3DLoaded() && c.HasKeyWord(requiredkeyword) 
		if !c.IsDead() && !c.IsBleedingOut() && !c.IsUnconscious()
			if (c.GetValue(Paralysis) != 1) && !c.IsInFaction(pTweakPosedFaction) && !c.IsDoingFavor()
				if (c.GetCurrentPackage() != pCommandMode_Travel) && (c.GetValue(pFollowerState) != 2.0)
	...
	c = pCompanion9.GetActorReference()
	if c && c.Is3DLoaded() && c.HasKeyWord(requiredkeyword) 
		if !c.IsDead() && !c.IsBleedingOut() && !c.IsUnconscious()
			if (c.GetValue(Paralysis) != 1) && !c.IsInFaction(pTweakPosedFaction) && !c.IsDoingFavor()
				if (c.GetCurrentPackage() != pCommandMode_Travel) && (c.GetValue(pFollowerState) != 2.0)
	...
	c = pCompanion10.GetActorReference()
	if c && c.Is3DLoaded() && c.HasKeyWord(requiredkeyword) 
		if !c.IsDead() && !c.IsBleedingOut() && !c.IsUnconscious()
			if (c.GetValue(Paralysis) != 1) && !c.IsInFaction(pTweakPosedFaction) && !c.IsDoingFavor()
				if (c.GetCurrentPackage() != pCommandMode_Travel) && (c.GetValue(pFollowerState) != 2.0)
...
bool Function LocalSendAffinityEvent(scriptobject Sender, keyword EventKeyword, objectReference Target = None, \
	...
	if (EventKeyword != CA_Event_HealCompanion)
		if CurrentAffinityEventData.NextDayAllowed > Utility.GetCurrentGameTime()
			...
			RETURN false
		endIf
	elseif (Target && Target.GetDistance(PlayerRef) < 180)
...
Function AffinityTalkManager(keyword EventKeyword, GlobalVariable  EventSize, bool CheckCompanionProximity, \
	...
	; int found = 0
	int slice = Utility.RandomInt(0, 9)	
	int i = slice
	while (i < 10)
...
Function AffinityTalkHelper(Actor npc, keyword EventKeyword, GlobalVariable  EventSize, \
	...
	if CheckCompanionProximity == true && npc.GetDistance(PlayerRef) > 2500
...
Function SetCompanion(Actor ActorToMakeCompanion, bool SetCompanion = true, bool FillCompanionAlias = true, bool SuppressDismissMessage = false, bool pSpinLock = false)
	...
	addKeywords(ActorToMakeCompanion)
...
Function RotateCompanion()
	...
	Actor p6 = pCompanion6.GetActorReference()
	if (p6)
		if !current
			Trace("p6 [" + p6.GetFactionRank(pTweakFollowerFaction) + "][" + p6.GetFactionRank(pTweakNamesFaction) + "]")
			pCompanion.ForceRefTo(p6)
			;AddKeywords(p6)
			return
		endIf
		if next_hit
	...
	Actor p7 = pCompanion7.GetActorReference()
	if (p7)
		if !current
			Trace("p7 [" + p7.GetFactionRank(pTweakFollowerFaction) + "][" + p7.GetFactionRank(pTweakNamesFaction) + "]")
			pCompanion.ForceRefTo(p7)
			;AddKeywords(p7)
			return
		endIf
		if next_hit
	...
	Actor p8 = pCompanion8.GetActorReference()
	if (p8)
		if !current
			Trace("p8 [" + p8.GetFactionRank(pTweakFollowerFaction) + "][" + p8.GetFactionRank(pTweakNamesFaction) + "]")
			pCompanion.ForceRefTo(p8)
			;AddKeywords(p8)
			return
		endIf
		if next_hit
	...
	Actor p9 = pCompanion9.GetActorReference()
	if (p9)
		if !current
			Trace("p9 [" + p9.GetFactionRank(pTweakFollowerFaction) + "][" + p9.GetFactionRank(pTweakNamesFaction) + "]")
			pCompanion.ForceRefTo(p9)
			;AddKeywords(p9)
			return
		endIf
		if next_hit
	...
	Actor p10 = pCompanion10.GetActorReference()
	if (p10)
		if !current
			Trace("p10 [" + p10.GetFactionRank(pTweakFollowerFaction) + "][" + p10.GetFactionRank(pTweakNamesFaction) + "]")
			pCompanion.ForceRefTo(p10)
			;AddKeywords(p10)
			return
		endIf
		if next_hit
...
Function LockRotation(Actor npc)
	...
	if npcRef
		...
		int currentLocID = PlayerRef.GetCurrentLocation().GetFormID()
...
Function UnLockRotation()
	...
	if npc
		...
		endIf
	endIf
	npc = pCompanion6.GetActorRef()
	if npc
		if npc.IsInFaction(pTweakRotateLockFollowerFaction)
			npc.RemoveFromFaction(pTweakRotateLockFollowerFaction)
		endIf
	endIf
	npc = pCompanion7.GetActorRef()
	if npc
		if npc.IsInFaction(pTweakRotateLockFollowerFaction)
			npc.RemoveFromFaction(pTweakRotateLockFollowerFaction)
		endIf
	endIf
	npc = pCompanion8.GetActorRef()
	if npc
		if npc.IsInFaction(pTweakRotateLockFollowerFaction)
			npc.RemoveFromFaction(pTweakRotateLockFollowerFaction)
		endIf
	endIf
	npc = pCompanion9.GetActorRef()
	if npc
		if npc.IsInFaction(pTweakRotateLockFollowerFaction)
			npc.RemoveFromFaction(pTweakRotateLockFollowerFaction)
		endIf
	endIf
	npc = pCompanion10.GetActorRef()
	if npc
		if npc.IsInFaction(pTweakRotateLockFollowerFaction)
			npc.RemoveFromFaction(pTweakRotateLockFollowerFaction)
		endIf
...
ReferenceAlias Function FindAlias(Actor npc)
	...
	elseif (pCompanion5.GetActorRef() == npc)
		return pCompanion5
	elseif (pCompanion6.GetActorRef() == npc)
		return pCompanion6
	elseif (pCompanion7.GetActorRef() == npc)
		return pCompanion7
	elseif (pCompanion8.GetActorRef() == npc)
		return pCompanion8
	elseif (pCompanion9.GetActorRef() == npc)
		return pCompanion9
	elseif (pCompanion10.GetActorRef() == npc)
		return pCompanion10
...
int Function CountFilledAliases()
	...
	if (pCompanion5.GetActorRef())
		ret += 1
	endIf
	if (pCompanion6.GetActorRef())
		ret += 1
	endIf
	if (pCompanion7.GetActorRef())
		ret += 1
	endIf
	if (pCompanion8.GetActorRef())
		ret += 1
	endIf
	if (pCompanion9.GetActorRef())
		ret += 1
	endIf
	if (pCompanion10.GetActorRef())
		ret += 1
	endIf
	return ret
endFunction
...
ReferenceAlias Function FindFreeAlias()
	...
	int followerLimit = pTweakFollowerLimit.GetValueInt()
	if 0 == followerLimit
		Trace("Bad TweakFollowerLimit. Trying Float")
		followerLimit = (pTweakFollowerLimit.GetValue() as Int)
		if 0 == followerLimit
			Trace("Bad TweakFollowerLimit. Trying Cast")
			GlobalVariable lTweakFollowerLimit = Game.GetFormFromFile(0x0400079D,"AmazingFollowerTweaks.esp") as GlobalVariable
			if lTweakFollowerLimit
				Trace("Found Global Variable")
				followerLimit = (lTweakFollowerLimit.GetValue() as Int)
			else
	...
	Trace("TweakFollowerLimit: [" + followerLimit + "]")

	ReferenceAlias[] aliasArray = new ReferenceAlias[10]
	aliasArray[0] = pCompanion1
	aliasArray[1] = pCompanion2
	aliasArray[2] = pCompanion3
	aliasArray[3] = pCompanion4
	aliasArray[4] = pCompanion5
	aliasArray[5] = pCompanion6
	aliasArray[6] = pCompanion7
	aliasArray[7] = pCompanion8
	aliasArray[8] = pCompanion9
	aliasArray[9] = pCompanion10

	int i = 0
	int usedFollowersCount = 0
	while (i < aliasArray.Length)
		if (aliasArray[i].GetReference())
			usedFollowersCount += 1
		endif
		i += 1
	endWhile

	if pTweakCharismaRecruifollowerLimit.getValueInt() == 1
		int charisma = PlayerRef.GetValue(pCharisma) as int
		;every 2 points of charisma is an allowed follower
		int charismaLimit = charisma / 2  ; Integer division rounds down
		if charisma == 1
			;for compatibility with vanilla dialogue, if you're charisma is 1 we will still allow you to to recruit
			charismaLimit = 1
		endif
		if usedFollowersCount >= charismaLimit
			debug.notification("Follower limit hit based on Charisma.")
			return None
		endif
	endif

	if (usedFollowersCount >= followerLimit)
		Trace("Follower limit reached. Used: " + usedFollowersCount + ", Limit: " + followerLimit)
		return None
	endIf

	; Now find and return the first free companion alias
	i = 0
	while (i < aliasArray.Length)
		if (!aliasArray[i].GetReference())
			Trace("Returning free alias at index " + i)
			return aliasArray[i]
		endif
		i += 1
	endWhile
	...
	Trace("Under limit but no free alias found.")
	return None
...
ReferenceAlias Function FindFilledAlias()
	...
	endIf
	ObjectReference p6 = pCompanion6.GetReference()
	if (p6)
		if (!(p6 as Actor).IsInFaction(pTweakRotateLockFollowerFaction))
			return pCompanion6
		else
			Trace("pCompanion6 is filled but is in the TweakRotatelockFollowerFaction")	
		endif
	else
		Trace("pCompanion6 appears empty")
	endIf
	ObjectReference p7 = pCompanion7.GetReference()
	if (p7)
		if (!(p7 as Actor).IsInFaction(pTweakRotateLockFollowerFaction))
			return pCompanion7
		else
			Trace("pCompanion7 is filled but is in the TweakRotatelockFollowerFaction")	
		endif
	else
		Trace("pCompanion7 appears empty")
	endIf
	ObjectReference p8 = pCompanion8.GetReference()
	if (p8)
		if (!(p8 as Actor).IsInFaction(pTweakRotateLockFollowerFaction))
			return pCompanion8
		else
			Trace("pCompanion8 is filled but is in the TweakRotatelockFollowerFaction")	
		endif
	else
		Trace("pCompanion8 appears empty")
	endIf
	ObjectReference p9 = pCompanion9.GetReference()
	if (p9)
		if (!(p9 as Actor).IsInFaction(pTweakRotateLockFollowerFaction))
			return pCompanion9
		else
			Trace("pCompanion9 is filled but is in the TweakRotatelockFollowerFaction")	
		endif
	else
		Trace("pCompanion9 appears empty")
	endIf
	ObjectReference p10 = pCompanion10.GetReference()
	if (p10)
		if (!(p10 as Actor).IsInFaction(pTweakRotateLockFollowerFaction))
			return pCompanion10
		else
			Trace("pCompanion10 is filled but is in the TweakRotatelockFollowerFaction")	
		endif
	else
		Trace("pCompanion10 appears empty")
	endIf
	...
	elseif (p5)
		...
		return pCompanion5
	elseif (p6)
		Trace("returning pCompanion6")
		return pCompanion6
	elseif (p7)
		Trace("returning pCompanion7")
		return pCompanion7
	elseif (p8)
		Trace("returning pCompanion8")
		return pCompanion8
	elseif (p9)
		Trace("returning pCompanion9")
		return pCompanion9
	elseif (p10)
		Trace("returning pCompanion10")
		return pCompanion10
	endIf
...
actor[] Function GetInfatuatedFollowers()
	...
	int found = 0
	int startSearch = Utility.RandomInt(0, 9)	
	int i = startSearch
	
	while (i < 10 && limit != found)
		npc = companions[i].GetActorRef()
		if npc
				...
				if Piper.IsInFaction(this.pCurrentCompanionFaction) == False
					...
					if (10 == Piper.GetValue(KellogDeathAV))
						...
						if (this.FindFreeAlias())
							...
							Float[] posdata = this.TraceCircle(pc, 150.0, 165)
						...
						Float[] posdata = this.TraceCircle(pc, 150.0, 180)
...
Function teleportCompanionsToInstPostQuest()
		ReferenceAlias[] followers = GetAllFollowers()
		int i = 0
		while (i < followers.length)
				Actor npc = followers[i].GetActorReference()
				...
				if !npc.isInFaction(pTweakHangoutFaction) && !( npc.getValue(Game.GetCommonProperties().FollowerState) == iFollower_Com_Wait.GetValue() )
						TeleportActorScript TeleportActor = npc as TeleportActorScript
						if TeleportActor
								pTweakFollowerScript.MoveToPlayer(npc, true, 180, false, 0, 0)
								;npc.queueUpdate()
								TeleportActor.teleportIn()
						else
...
Function teleportCompanionsFrominstPostQuest(Actor companion)
		if !companion.isInFaction(pTweakHangoutFaction) && !( companion.getValue(Game.GetCommonProperties().FollowerState) == iFollower_Com_Wait.GetValue() )
				TeleportActorScript TeleportActor = companion as TeleportActorScript
				if TeleportActor
						TeleportActor.teleportIn()
				else
						debug.trace("No teleportactorscript attached to companion, won't apply teleport FX going out of institute/")
				endif
				;companion.queueUpdate()
		endif

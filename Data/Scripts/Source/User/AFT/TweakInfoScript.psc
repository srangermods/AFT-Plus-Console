...
; 8 = Keywords
; 9 = Reactions
Function Setup(int type = 0, Actor passed_in_npc = None)
	Trace("Setup(" + type + ")")
	npc = passed_in_npc
...
Function UpdateRelationship(Actor passed_in_npc, ObjectReference theBook)
	Trace("UpdateRelationship()")
	; Actor npc = pInfoNPC.GetActorReference()
	; ObjectReference theBook =  pInfoBook.GetReference()
	
	if !passed_in_npc || !theBook
		Trace("npc or book is empty. Aborting")
		return		
	endif
	
	CompanionActorScript cas = passed_in_npc as CompanionActorScript
	if cas
		...
		float currentAffinity  = passed_in_npc.GetValue(pCA_Affinity)
		float currentThreshold = passed_in_npc.GetValue(pCA_CurrentThreshold)
...
Function UpdateReactions(Actor passed_in_npc, ObjectReference theBook)
	...
	bool NoDisapprove	= passed_in_npc.IsInFaction(pTweakNoDisapprove)
	bool NoApprove		= passed_in_npc.IsInFaction(pTweakNoApprove)
	bool ConvNegToPos	= passed_in_npc.IsInFaction(pTweakConvNegToPos)
	bool ConvPosToNeg	= passed_in_npc.IsInFaction(pTweakConvPosToNeg)
...
Function UpdateEffects(Actor passed_in_npc, ObjectReference theBook)
	Trace("UpdateEffects()")
	; ObjectReference theBook =  pInfoBook.GetReference()
	...
	while (i < numspellstocheck && tokenindex < maxdisplayeffects)
		...
		if (passed_in_npc.HasSpell(stest))
	...
	while (i < numeffectstocheck && tokenindex < maxdisplayeffects)
		...
		if (passed_in_npc.HasMagicEffect(mtest))
...
Function UpdateKeywords(Actor passed_in_npc, ObjectReference theBook)
	...
	while (i < numkeywordstocheck && tokenindex < maxdisplaykeywords)
		...
		if (passed_in_npc.HasKeyword(ktest))
...
Function UpdatePerks(Actor passed_in_npc, ObjectReference theBook)
	Trace("UpdatePerks()")
	; ObjectReference theBook =  pInfoBook.GetReference()
	...
	while (i < numperkstocheck && tokenindex < maxdisplayperks)
		...
		if (passed_in_npc.HasPerk(ptest))

...
Message          Property pEnableAftSpouseMessage       Auto Const
...
Bool isAFTSpouseEnabled = true

Bool Function EnableAFTSpouse()
	Utility.wait(2)
	int iButton = pEnableAftSpouseMessage.Show(0, 0, 0, 0, 0, 0, 0, 0, 0)
	if iButton  == 1
		return true
	else
		...
		RemoveInventoryEventFilter(pArmor_SpouseWeddingRing)
		return false
	endif
EndFunction
...
Event OnQuestInit()
	...
	;for those who installed AFT Plus not in a new game
...
Function UpdateOccupants()
	...
	if (MQ205.GetCurrentStageID() > 9 && GetCurrentStageID() < 30)
...
Event OnDistanceLessThan(ObjectReference akPlayer, ObjectReference akPod, float afDistance)
	...
	; Have to rescue her before meeting and talking to Virgil in the Glowing Sea
	if (MQ205.GetCurrentStageID() > 9)

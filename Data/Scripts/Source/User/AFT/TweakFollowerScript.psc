...
Scene Property Dogmeat_FindItem_Scene Auto Const
Scene Property Dogmeat_FindContainer_Scene Auto Const
...
GlobalVariable Property InstDoneSummonMsgOnlyOnce       Auto Const
EndGroup
...
Message Property pTweakInstituteDoneSummonMsg Auto Const
...
Keyword    Property p_AttachSlot1           Auto Const
...
Function OnGameLoaded(bool firstTime=false)
	...
	elseif (1.23 != version && version < 3.03)
...
bool Function GetCompanionsInCombat()
	...
	if  pCompanion6 && pCompanion6.GetReference()
		tally += 1
		if pCompanion6.GetActorRef().IsInCombat()
			return true
		endif
	endif
	if  pCompanion7 && pCompanion7.GetReference()
		tally += 1
		if pCompanion7.GetActorRef().IsInCombat()
			return true
		endif
	endif
	if  pCompanion8 && pCompanion8.GetReference()
		tally += 1
		if pCompanion8.GetActorRef().IsInCombat()
			return true
		endif
	endif
	if  pCompanion9 && pCompanion9.GetReference()
		tally += 1
		if pCompanion9.GetActorRef().IsInCombat()
			return true
		endif
	endif
	if  pCompanion10 && pCompanion10.GetReference()
		tally += 1
		if pCompanion10.GetActorRef().IsInCombat()
			return true
		endif
	endif
...
Function EventFollowerRecruited(Actor npc)
	...
	if (a && a.GetActorRef())
		...
		elseif npc == pDogMeatCompanion.GetActorRef()
			npc.AddKeyword(pPlayerCanStimpak)
		else
...
Function EventPlayerEnterVertibird()
	...
	int pFollowerMapLength = pFollowerMap.length	
	Actor npc
	Keyword[] kSeats = new Keyword[0]
	;p_AttachPassenger
	kSeats.add(p_AttachPassenger) ;passenger 0
	kSeats.add(p_AttachCoPilot) ;copilot 1
	kSeats.add(p_AttachSlot1) ;left standing 2 p-attachslot1 (companion power armor slot + codsworth)
	kSeats.add(p_AttachSlot2) ;right standing 3 p-attachslot2
	int kSeatsLength = kSeats.length
	bool keywordFound = false
	while (i < pFollowerMapLength)
		npc = pFollowerMap[i].GetActorRef()
		if (npc)
			;check if npc has both passenger + one other
			if npc.hasKeyword(kseats[0])
				while (j < kSeatsLength && !keywordFound)
					if npc.hasKeyword(kseats[j])
						npc.removeKeyword(kseats[0])
						keywordFound = true
					endif
					j += 1
				endWhile
...
Function EventPlayerWeaponDraw()
	...
	if (pTweakFollowerCatchup.GetValue() == 1.0)
		...
		if npc == pDogmeatCompanion.GetActorRef()
			Actor dogmeatRef = pDogmeatCompanion.GetActorRef()
			if dogmeatRef.GetCurrentScene() == Dogmeat_FindItem_Scene
				Dogmeat_FindItem_Scene.stop()
				;debug.notification("stopped Dogmeat_FindItem_Scene!")
			elseif dogmeatRef.GetCurrentScene() == Dogmeat_FindContainer_Scene
				Dogmeat_FindContainer_Scene.stop()
				;debug.notification("stopped Dogmeat_FindContainer_Scene!")
			endif
		endif
		npc.evaluatePackage(true)
	endif
...
Function EventPlayerWeaponSheath()
	...
	while (i < pFollowerMapLength)
		...
		if (npc)
			...
			Utility.wait(1.5)
			if npc.isWeaponDrawn() && !npc.isInCombat()
				Trace("Companion " + npc + "still has weapon drawn when player unsheathed.")
...
Function FollowerInfo(Actor npc, int type = 0)
	...
	if (pTweakInfoScript)
		...
		pTweakInfoScript.Setup(type, npc)
		if (0 == type || 7 == type)
			if s
...
Event Quest.OnStageSet(Quest pQuest, int auiStageID, int auiItemID)
	...
	elseif DLC04RaiderKickout == pQuest
		if (20 == auistageID)
			UnRegisterForRemoteEvent(DLC04RaiderKickout,"OnStageSet")
			;handle nuka world raider kickout

			ActorBase porterGage = Game.getFormFromFile(0x00881D,"DLCNukaWorld.esm") as actorbase
			int plength = pManagedMap.Length
			int p = 1
			while (p < plength)
				...
				if pfix
					...
					if pa.getActorBase() == porterGage
						if !pa.IsDead()
							if pa.IsInFaction(pTweakFollowerFaction)
								debug.trace("Unmanaging Porter Gage from AFT")
								UnManageFollower(pa)
								...
								pa.SetGhost(false)
								pa.setrelationshiprank(Game.GetPlayer(),0)
...
Event Quest.OnQuestInit(Quest auiQuest)
	...
	elseif DLC04RaiderKickout == auiQuest
		RegisterForRemoteEvent(DLC04RaiderKickout, "OnStageSet")	
	else
...
; WeaponDraw Event -> Summon All Followers (behind you) - Does not include Waiting
...
Function MoveToPlayer(Actor npc, bool excludeWaiting=false, float startingOffset=180.0, bool excludeInCombat=false, float outsideradius = 0.0, float cylinder_height = 0.0, bool TeleportToHere = false)
	...
	trace(playerref + "'s current location: "+pLoc)
	if inst308.isCompleted()
		if (InstDoneSummonMsgOnlyOnce.getValue()==0.0)
			pTweakInstituteDoneSummonMsg.Show()
			InstDoneSummonMsgOnlyOnce.setValue(1.0)
		endif
	endif	
	if (!npc && 0.0 == MQWonInst.GetValue() && !pDN136_Attack.GetStageDone(10) && InstituteLocation.isChild(pLoc) && !inst308.isCompleted() )			
	...
	if (npc)
		if (!excludeInCombat || !npc.IsInCombat())
			...
			if (fLoc && pLoc && fLoc.IsSameLocation(pLoc))
				...
				posdata=TraceCircle(playerref, 125, npcPlaceAngle)		
				...
				posdata=TraceCircleOffsets(playerref, 125, npcPlaceAngle)		
				npc.MoveTo(playerref, posdata[0], posdata[1], posdata[2], false)
				if TeleportToHere
					npc.AddSpell(TeleportInSpell)
				endif			
			endif
			npc.SetAngle(ax, ay, npc.GetAngleZ() + npc.GetHeadingAngle(PlayerRef))
		else	
			...
			if (!excludeDog || ( pFollowerMap[i] != pDogmeatCompanion))
				if (!excludeNonCore || IsCoreCompanion(npc))
					if (!excludeWaiting || ( npc.GetCurrentPackage() != pCommandMode_Travel && npc.GetValue(pFollowerState) != ComWait))
						if (0 == outsideradius || !DistanceWithinCylinder(playerref,npc,outsideradius, cylinder_height))
							int id = npc.GetFactionRank(pTweakFollowerFaction) As Int	
							if (id > 0)
...
Function initializeFollowerMap()
	...
	pFollowerMap.Add(pCompanion6)
	pFollowerMap.Add(pCompanion7)
	pFollowerMap.Add(pCompanion8)
	pFollowerMap.Add(pCompanion9)
	pFollowerMap.Add(pCompanion10)
...
Function RegisterForKickOut()
	...
	UnRegisterForRemoteEvent(MinDestBoS,"OnStageSet") 
	UnRegisterForRemoteEvent(DLC04RaiderKickout	, "OnQuestInit")
	UnRegisterForRemoteEvent(DLC04RaiderKickout	, "OnStageSet")
	...
	if !DLC04RaiderKickout.IsRunning()
		if 0 == DLC04RaiderKickout.GetCurrentStageID()
			RegisterForRemoteEvent(DLC04RaiderKickout,"OnQuestInit")
		endif
	elseif !DLC04RaiderKickout.GetStageDone(255)
		debug.trace("Registering for DLC04RaiderKickout OnStageSet")
		RegisterForRemoteEvent(DLC04RaiderKickout,"OnStageSet")
	endif		

	...
	if !RegisteredEpisode
		if (auiQuest == EpisodeQuest)
			...
			if (EpisodeQuestStage < 1) 				
				...
				RegisterForRemoteEvent(EpisodeQuest,"OnStageSet")
				;edge case where Stage is set before RegisterForRemoteEvent happens, we want to register here:
				RegisterInterjectionInfo()	
			endif
			return
		endif
...
Event Scene.OnBegin(Scene auiScene)
	...
	if (-1 != Persisted.FindStruct("_theScene", auiScene))
		...
		ReferenceAlias[] companions = None
		if (ExternalFile ==  "SS2_XPAC_Chapter2.esm" || ExternalFile ==  "SS2.esm")
			companions = pTweakFollowerScript.GetAllTweakFollowers(excludeWaiting=true, excludeDog=false, excludeNonCore=false)
		else
			companions = pTweakFollowerScript.GetAllTweakFollowers(excludeWaiting=true, excludeDog=true, excludeNonCore=false)
		endif
...
Event Scene.OnEnd(Scene auiScene)
	...
	if auiScene == (Game.GetForm(0x001091E4) as Scene) || auiScene == (Game.GetForm(0x00028658) as Scene || auiScene == (Game.GetForm(0x0014A132)) as Scene) ;GrayGardenIntro and MS01BillyOutOfFridge and DogmeatLostTrail
		return
	endif
	Trace("  UnRegistering Events [" + auiScene + "]->OnPhaseBegin, OnPhaseEnd, OnEnd")
	;accidentally leaving a scene, dont unregister unless commentary was handled
	int lookupindex = Persisted.FindStruct("_theScene", auiScene)
	if Persisted[lookupindex]._processed == true
		Trace("  UnRegistering Events [" + auiScene + "]-> OnBegin")
		UnregisterForRemoteEvent(auiScene, "OnBegin")
		Trace("  UnRegistering Events [" + auiScene + "]->OnPhaseBegin, OnPhaseEnd, OnEnd")
...
Function HandleCommentary(InterjectionInfo info)
	...
	if info.Topic1ID > -1		
		if info.Topic1ID > 0
			...
			if !ptopic1
				Trace("  Warning: Default VANILLA Topic unavailable. Looking for MOD topic now")
				...
				if pchoice2 && pchoice2.HasBeenSaid() && 0 != info.Topic2ID
					...
					if info.Topic2ID > -1
						...
						elseif info.Topic2ID > 0
							...
							stopic2 = Game.GetForm(info.Spouse2ID) as Topic
						endif
					endIf
					theTopic    = ptopic2
					spouseTopic = stopic2
	...
	while (i < numcompanions)
		if i != inscene
			...
			Trace("  Having Companion [" + i + "]: "+ akSpeakerRef +" Speak Topic [" + theTopic + "]")
			...
			Utility.wait(0.1)
			if pCenterCameraTarget && akSpeakerRef.IsTalking()

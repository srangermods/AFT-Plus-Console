...
Event OnEnterBleedout()
	...
	while (i < candidatesLen)
		...
		if (test != npc)		
			if (!test.IsDead() && (test.GetBaseObject() as ActorBase).isUnique() && !test.IsBleedingOut() && !test.IsUnconscious() && (test.GetValue(Paralysis) != 1) && !test.IsInFaction(pTweakPosedFaction) && !test.IsDoingFavor() && test.GetCurrentPackage() != pCommandMode_Travel && test.GetValue(pFollowerState) != ComWait) 
...
Function HealAll(Actor myPatient)
	Trace("HealAll()")
	myPatient.SetNoBleedoutRecovery(false)
	myPatient.StopCombat()
	Trace("Allow Companion to recover from Bleedout")

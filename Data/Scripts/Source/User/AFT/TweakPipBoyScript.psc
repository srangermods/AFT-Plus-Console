...
Function ImportNPCRelay(int itype = 0)
	...
	if TerminalTarget.getActorBase() == (Game.GetFormFromFile(0x0000AB33, "llamaCompanionHeatherv2.esp") as ActorBase)
		debug.notification("Heather is not meant to be imported into AFT!")
		return
	endif
...
Function setVertibirdSeat(int seat)
	Keyword[] kSeats = new Keyword[0]
	kSeats.add(Game.GetFormFromFile(2070617, "Fallout4.esm") as Keyword, 1) ;passenger 0
	kSeats.add(Game.GetFormFromFile(1416296, "Fallout4.esm") as Keyword, 1) ;copilot 1
	kSeats.add(Game.GetFormFromFile(1044876, "Fallout4.esm") as Keyword, 1) ;left standing 2 p-attachslot1 (companion power armor slot + codsworth)
	kSeats.add(Game.GetFormFromFile(1044877, "Fallout4.esm") as Keyword, 1) ;right standing 3
	Utility.waitmenumode(0.1)
	if TerminalTarget
		trace(TerminalTarget + " getting assigned vertibird keyword.")
		;assigned keyword is the same the companion already has
		;remove keyword and add the new one
		int i = 0
		while (i < kSeats.length)
			if TerminalTarget.HasKeyword(kSeats[i])
				Trace("Removing vertibird keyword: " +kSeats[i])
				TerminalTarget.removeKeyword(kSeats[i])
			endif
			i=i+1
		endwhile
		Trace("Adding vertibird keyword: " +kSeats[seat])
		TerminalTarget.addKeyword(kSeats[seat])
	else
		trace("No TerminalTarget for Vertibird seat to set.")
	endif

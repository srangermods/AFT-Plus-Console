...
Location Property instLocation Auto Const
...
Event Actor.OnLocationChange(Actor akSender, Location akOldLoc, Location akNewLoc)
	if akSender == playerRef
		...
		if inst308.isCompleted()
			if instLocation.isChild(akNewLoc) && !instLocation.isChild(akOldloc)
				AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
				if DFScript
					DFScript.teleportCompanionsToInstPostQuest()
				endif
			endif
		endif
	;for the other companions
	elseif !instLocation.isChild(akNewLoc) && instLocation.isChild(akOldloc) && inst308.isCompleted()
		;apply the teleport effect
		AFT:TweakDFScript DFScript = ((self as Quest) as AFT:TweakDFScript)
		if DFScript
			DFScript.teleportCompanionsFrominstPostQuest(akSender)
		endif
	endif
EndEvent
...
Event OnHit(ObjectReference RemoteSource, ObjectReference akAggressor, Form akSource, Projectile akProjectile, bool abPowerAttack, bool abSneakAttack, bool abBashAttack, bool abHitBlocked, string asMaterialName)
	...
	if RemoteSource == playerRef
		...
		elseif (pCompanion5 && pCompanion5.GetActorRef() == npc)
			HandleCombatMessageToSituationAwareness(akAggressor as Actor)			
		elseif (pCompanion6 && pCompanion6.GetActorRef() == npc)
			HandleCombatMessageToSituationAwareness(akAggressor as Actor)			
		elseif (pCompanion7 && pCompanion7.GetActorRef() == npc)
			HandleCombatMessageToSituationAwareness(akAggressor as Actor)			
		elseif (pCompanion8 && pCompanion8.GetActorRef() == npc)
			HandleCombatMessageToSituationAwareness(akAggressor as Actor)			
		elseif (pCompanion9 && pCompanion9.GetActorRef() == npc)
			HandleCombatMessageToSituationAwareness(akAggressor as Actor)			
		elseif (pCompanion10 && pCompanion10.GetActorRef() == npc)
			HandleCombatMessageToSituationAwareness(akAggressor as Actor)			
		endif
	endif
	...
	RegisterForHitEvent(pCompanion6)
	RegisterForHitEvent(pCompanion7)
	RegisterForHitEvent(pCompanion8)
	RegisterForHitEvent(pCompanion9)
	RegisterForHitEvent(pCompanion10)
	...
	if akTarget
		if RemoteSource == playerRef
			...
			elseif (pCompanion5 && RemoteSource == pCompanion5.GetActorReference())
				HandleCombatMessageToSituationAwareness(akTarget as Actor)
			elseif (pCompanion6 && RemoteSource == pCompanion6.GetActorReference())
				HandleCombatMessageToSituationAwareness(akTarget as Actor)
			elseif (pCompanion7 && RemoteSource == pCompanion7.GetActorReference())
				HandleCombatMessageToSituationAwareness(akTarget as Actor)
			elseif (pCompanion8 && RemoteSource == pCompanion8.GetActorReference())
				HandleCombatMessageToSituationAwareness(akTarget as Actor)
			elseif (pCompanion9 && RemoteSource == pCompanion9.GetActorReference())
				HandleCombatMessageToSituationAwareness(akTarget as Actor)
			elseif (pCompanion10 && RemoteSource == pCompanion10.GetActorReference())
				HandleCombatMessageToSituationAwareness(akTarget as Actor)
			endif			
		endif
	endif
...
Event Actor.OnPlayerHealTeammate(Actor akSender, Actor akTeammate)
	...
	if akTeammate == DogmeatCompanion.GetActorReference()
		...
		elseif (pCompanion6 && akTeammate == pCompanion6.GetActorReference())
			SendAffinityEvent(self, HealCompanion, pCompanion6.GetActorReference(), None, true)
		elseif (pCompanion7 && akTeammate == pCompanion7.GetActorReference())
			SendAffinityEvent(self, HealCompanion, pCompanion7.GetActorReference(), None, true)
		elseif (pCompanion8 && akTeammate == pCompanion8.GetActorReference())
			SendAffinityEvent(self, HealCompanion, pCompanion8.GetActorReference(), None, true)
		elseif (pCompanion9 && akTeammate == pCompanion9.GetActorReference())
			SendAffinityEvent(self, HealCompanion, pCompanion9.GetActorReference(), None, true)
		elseif (pCompanion10 && akTeammate == pCompanion10.GetActorReference())
			SendAffinityEvent(self, HealCompanion, pCompanion10.GetActorReference(), None, true)
		endif
	endif
EndEvent
...
Event Actor.OnPlayerSwimming(Actor akSender)
	trace(self, "OnPlayerSwimming()" + PlayerRef.GetCurrentLocation())
	HandleOnSwimming(PlayerRef.GetCurrentLocation())
...
Event OnRadiationDamage(ObjectReference akTarget, bool abIngested)
	...
	if PlayerRef.IsInCombat() == false && abIngested == false
...
Event ReferenceAlias.OnCommandModeGiveCommand(ReferenceAlias akSender, int aeCommandType, ObjectReference akTarget)
	...
	if akSender == DogmeatCompanion && aeCommandType == 6
		...
		elseif (pCompanion5 && SenderActor == pCompanion5.GetActorReference())
			bFoundCommand = true
		elseif (pCompanion6 && SenderActor == pCompanion6.GetActorReference())
			bFoundCommand = true
		elseif (pCompanion7 && SenderActor == pCompanion7.GetActorReference())
			bFoundCommand = true
		elseif (pCompanion8 && SenderActor == pCompanion8.GetActorReference())
			bFoundCommand = true
		elseif (pCompanion9 && SenderActor == pCompanion9.GetActorReference())
			bFoundCommand = true
		elseif (pCompanion10 && SenderActor == pCompanion10.GetActorReference())
			bFoundCommand = true
		endif
		if 	(bFoundCommand)
			if (akTarget)
...
;    9 - Heal 
	...
	if akSender == DogmeatCompanion && aeCommandType == 6 && !PlayerRef.isSneaking()
...
bool Function StartCompanionInteractionScene(GlobalVariable SceneTypeGlobal, objectReference SceneRef, location SceneLoc, Actor CompanionActor = None) global
	...
	If Game.IsPluginInstalled("AmazingFollowerTweaks.esp")
		...
		if selfQuest
			...
			ReferenceAlias gv_Companion6 = (selfQuest.GetAlias(24) as ReferenceAlias)
			ReferenceAlias gv_Companion7 = (selfQuest.GetAlias(25) as ReferenceAlias)
			ReferenceAlias gv_Companion8 = (selfQuest.GetAlias(26) as ReferenceAlias)
			ReferenceAlias gv_Companion9 = (selfQuest.GetAlias(27) as ReferenceAlias)
			ReferenceAlias gv_Companion10 = (selfQuest.GetAlias(28) as ReferenceAlias)						
			...
			Actor gv_CompanionActor6 = gv_Companion6.GetActorReference()
			Actor gv_CompanionActor7 = gv_Companion7.GetActorReference()
			Actor gv_CompanionActor8 = gv_Companion8.GetActorReference()
			Actor gv_CompanionActor9 = gv_Companion9.GetActorReference()
			Actor gv_CompanionActor10 = gv_Companion10.GetActorReference()

			Actor[] companionActors = new Actor[0]
			if gv_CompanionActor1
				companionActors.add(gv_CompanionActor1)
			endif
			if gv_CompanionActor2
				companionActors.add(gv_CompanionActor2)
			endif
			if gv_CompanionActor3
				companionActors.add(gv_CompanionActor3)
			endif
			if gv_CompanionActor4
				companionActors.add(gv_CompanionActor4)
			endif
			if gv_CompanionActor5
				companionActors.add(gv_CompanionActor5)
			endif
			if gv_CompanionActor6
				companionActors.add(gv_CompanionActor6)
			endif
			if gv_CompanionActor7
				companionActors.add(gv_CompanionActor7)
			endif
			if gv_CompanionActor8
				companionActors.add(gv_CompanionActor8)
			endif
			if gv_CompanionActor9
				companionActors.add(gv_CompanionActor9)
			endif
			if gv_CompanionActor10
				companionActors.add(gv_CompanionActor10)
			endif

			int i = 0
			bool ret = true
			bool trycompanion = true	
			while (i < companionActors.length)
				
				trace(selfScript, "AFT StartCompanionInteractionScene()" + SceneTypeGlobal + ", " + SceneRef + ", " + SceneLoc + ", " + companionActors[i] )
				ret = ret && selfScript.CIS_Companion_Scene_Start.SendStoryEventAndWait(akLoc = SceneLoc, akRef1 = companionActors[i], akRef2 = SceneRef, aiValue1 = SceneTypeGlobal.GetValue() as int)
				trycompanion = false
				Utility.wait(0.1)
				i +=1
			endwhile
...
Function HandleTimer_LoiterSample()
	...
	if shouldEVPFollowers
		...
		if DFScript
			...
			if pCompanion6
				test = pCompanion6.GetActorReference()
				if test && !test.IsDoingFavor()
					if (now || Utility.RandomInt(0, 100) > 25)
						test.EvaluatePackage()
					endif
			...
			if pCompanion7
				test = pCompanion7.GetActorReference()
				if test && !test.IsDoingFavor()
					if (now || Utility.RandomInt(0, 100) > 25)
						test.EvaluatePackage()
					endif
			...
			if pCompanion8
				test = pCompanion8.GetActorReference()
				if test && !test.IsDoingFavor()
					if (now || Utility.RandomInt(0, 100) > 25)
						test.EvaluatePackage()
					endif
			...
			if pCompanion9
				test = pCompanion9.GetActorReference()
				if test && !test.IsDoingFavor()
					if (now || Utility.RandomInt(0, 100) > 25)
						test.EvaluatePackage()
					endif
			...
			if pCompanion10
				test = pCompanion10.GetActorReference()
				if test && !test.IsDoingFavor()
					if (now || Utility.RandomInt(0, 100) > 25)
						test.EvaluatePackage()
					endif
		...
		if (akSender as Actor) == PlayerRef
			...
			if akSenderRef
				...
				elseif akSenderRef == pCompanion5
					SendAffinityCrippledEvent(akSenderRef.GetActorReference(), akActorValue, isCompanion = true)
				elseif akSenderRef == pCompanion6
					SendAffinityCrippledEvent(akSenderRef.GetActorReference(), akActorValue, isCompanion = true)
				elseif akSenderRef == pCompanion7
					SendAffinityCrippledEvent(akSenderRef.GetActorReference(), akActorValue, isCompanion = true)
				elseif akSenderRef == pCompanion8
					SendAffinityCrippledEvent(akSenderRef.GetActorReference(), akActorValue, isCompanion = true)
				elseif akSenderRef == pCompanion9
					SendAffinityCrippledEvent(akSenderRef.GetActorReference(), akActorValue, isCompanion = true)
				elseif akSenderRef == pCompanion10
					SendAffinityCrippledEvent(akSenderRef.GetActorReference(), akActorValue, isCompanion = true)
				endif
			endif
		endif
...
Function HandleOnItemEquipped(ObjectReference akSender, Form akBaseObject)
	...
	if akSender == PlayerRef && akBaseObject.HasKeyword(isPowerArmorFrame)
...
ReferenceAlias pCompanion6
ReferenceAlias pCompanion7
ReferenceAlias pCompanion8
ReferenceAlias pCompanion9
ReferenceAlias pCompanion10
...
Function initialize()
	if needs_initialize
		...
		pCompanion6           = ((self as Quest).GetAlias(24) as ReferenceAlias)
		pCompanion7           = ((self as Quest).GetAlias(25) as ReferenceAlias)
		pCompanion8           = ((self as Quest).GetAlias(26) as ReferenceAlias)
		pCompanion9           = ((self as Quest).GetAlias(27) as ReferenceAlias)
		pCompanion10           = ((self as Quest).GetAlias(28) as ReferenceAlias)
...
Function RegisterEvents()
	...
	RegisterForRemoteEvent(pCompanion1.getActorReference(), "OnLocationChange")
	RegisterForRemoteEvent(pCompanion2.getActorReference(), "OnLocationChange")
	RegisterForRemoteEvent(pCompanion3.getActorReference(), "OnLocationChange")
	RegisterForRemoteEvent(pCompanion4.getActorReference(), "OnLocationChange")
	RegisterForRemoteEvent(pCompanion5.getActorReference(), "OnLocationChange")
	RegisterForRemoteEvent(pCompanion6.getActorReference(), "OnLocationChange")
	RegisterForRemoteEvent(pCompanion7.getActorReference(), "OnLocationChange")
	RegisterForRemoteEvent(pCompanion8.getActorReference(), "OnLocationChange")
	RegisterForRemoteEvent(pCompanion9.getActorReference(), "OnLocationChange")
	RegisterForRemoteEvent(pCompanion10.getActorReference(), "OnLocationChange")
	RegisterForRemoteEvent(DogmeatCompanion.getActorReference(), "OnLocationChange")
	...
	RegisterForRemoteEvent(pCompanion6, "OnCripple") ;used by the Affinity Event system
	RegisterForRemoteEvent(pCompanion7, "OnCripple") ;used by the Affinity Event system
	RegisterForRemoteEvent(pCompanion8, "OnCripple") ;used by the Affinity Event system
	RegisterForRemoteEvent(pCompanion9, "OnCripple") ;used by the Affinity Event system
	RegisterForRemoteEvent(pCompanion10, "OnCripple") ;used by the Affinity Event system
	...
	RegisterForRemoteEvent(pCompanion6, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	RegisterForRemoteEvent(pCompanion7, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	RegisterForRemoteEvent(pCompanion8, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	RegisterForRemoteEvent(pCompanion9, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	RegisterForRemoteEvent(pCompanion10, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	...
	RegisterForHitEvent(pCompanion6)  ; used by: 	"SITUATION AWARENESSS" & "FOLLOWER, COMPANION, COMMANDS"
	RegisterForHitEvent(pCompanion7)  ; used by: 	"SITUATION AWARENESSS" & "FOLLOWER, COMPANION, COMMANDS"
	RegisterForHitEvent(pCompanion8)  ; used by: 	"SITUATION AWARENESSS" & "FOLLOWER, COMPANION, COMMANDS"
	RegisterForHitEvent(pCompanion9)  ; used by: 	"SITUATION AWARENESSS" & "FOLLOWER, COMPANION, COMMANDS"
	RegisterForHitEvent(pCompanion10)  ; used by: 	"SITUATION AWARENESSS" & "FOLLOWER, COMPANION, COMMANDS"
...
Function AftReset()
	...
	UnRegisterForRemoteEvent(pCompanion6, "OnCripple") ;used by the Affinity Event system
	UnRegisterForRemoteEvent(pCompanion7, "OnCripple") ;used by the Affinity Event system
	UnRegisterForRemoteEvent(pCompanion8, "OnCripple") ;used by the Affinity Event system
	UnRegisterForRemoteEvent(pCompanion9, "OnCripple") ;used by the Affinity Event system
	UnRegisterForRemoteEvent(pCompanion10, "OnCripple") ;used by the Affinity Event system
	...
	UnRegisterForRemoteEvent(pCompanion6, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	UnRegisterForRemoteEvent(pCompanion7, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	UnRegisterForRemoteEvent(pCompanion8, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	UnRegisterForRemoteEvent(pCompanion9, "OnCommandModeGiveCommand") ;used to start quests for specific behavior
	UnRegisterForRemoteEvent(pCompanion10, "OnCommandModeGiveCommand") ;used to start quests for specific behavior	
